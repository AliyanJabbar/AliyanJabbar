...
const FallingText: React.FC<FallingTextProps> = ({
  ...
  useEffect(() => {
    ...
    const newHTML = words
      .map((word) => {
        ...
        return `<span
          class="inline-block mx-[2px] select-none text-dark dark:text-light ${
            isHighlighted ? "text-primary dark:text-primaryDark font-bold" : ""
          }"
        >
          ${word}
        </span>`;
  ...
  useEffect(() => {
    ...
    // Adjusted boundaries to keep text in view
    const floor = Bodies.rectangle(
      width / 2,
      height + 5, // Closer floor
      width,
      50,
      boundaryOptions
    );
